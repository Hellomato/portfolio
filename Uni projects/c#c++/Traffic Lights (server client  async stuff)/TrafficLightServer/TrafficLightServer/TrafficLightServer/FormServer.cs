...
namespace TrafficLightServer
{
    ...
    public partial class FormServer : Form
    {
        ...
        private int[] carsArray = new int[2];
        ...
        public void MessageReceived(Object received)
        {
            ...
            if(message.Contains("Car arrive"))
            {
                carsArray[0] += 1;
                listBoxOutput.Items.Add(carsArray[0]);

                if(carsArray[0] >= 10)
                {
                    sendString("Green", textBoxLightIP.Text);
                }
            }else if(message.Contains("Car left"))
            {
                if (carsArray[0] == 0)
                {
                    return;
